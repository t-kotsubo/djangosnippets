...
class SnippetDetailTest(TestCase):
    def setUp(self):
        ...
        self.snipet = Snippet.objects.create(title="タイトル", code="コード",
                                             description="解説", created_by=self.user)
    ...
    def test_top_page_returns_200_and_expected_heading(self):
        response = self.client.get("/snippets/%s/" % self.snipet.id)
        self.assertContains(response, self.snipet.title, status_code=200)


class CreateSnipperTest(TestCase):
    def setUp(self):
